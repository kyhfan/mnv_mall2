...
function change_banner_sort(banner_type)
{
	$.ajax({
		type   : "POST",
		async  : false,
		url    : "admin_exec.php",
		data:{
			"exec"	: "change_banner_list",
			"banner_type"	: banner_type
		},
		success: function(response){
			$("#banner_list").html(response);
		}
	});	
}

function change_banner_device_type(banner_type)
{
	$(".device_type_class").hide();
	$("#"+banner_type+"_device_type").show();
}
