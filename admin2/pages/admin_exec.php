	...
	switch ($_REQUEST['exec'])
	{
		...
		case "show_event_list" :
			...
			while ($list_data = mysqli_fetch_array($list_result))
			{
				...
				$innerHTML	.= "<td><a href='./event_detail.php?mode=view&idx=".$list_data['idx']."'><button type='button' class='btn btn-primary'>보기</button></a> <a href='#' onclick='delete_event(".$list_data['idx'].");return false;'><button type='button' class='btn btn-danger'>삭제</button></a></td>";
				$innerHTML	.= "</tr>";
				//$i++;
			}
			$innerHTML	.= "</tbody>";
		...
		case "show_post_list" :
			...
			while ($list_data = mysqli_fetch_array($list_result))
			{
				...
				$innerHTML	.= "<td><a href='./post_detail.php?mode=view&idx=".$list_data['idx']."'><button type='button' class='btn btn-primary'>보기</button></a> <a href='#' onclick='delete_post(".$list_data['idx'].");return false;'><button type='button' class='btn btn-danger'>삭제</button></a></td>";
				$innerHTML	.= "</tr>";
				//$i++;
			}
			$innerHTML	.= "</tbody>";
		...
		case "show_stock_list" :
			...
			while ($list_data = mysqli_fetch_array($list_result))
			{
				...
				$innerHTML	.= "<td>".$current_stock." 원</td>";
				$innerHTML	.= "<td><a href='./stock_detail.php?goodscode=".$list_data['goods_code']."'><button type='button' class='btn btn-primary'>수정</button></a></td>";
		...
		case "insert_sales_store_info" :
			...
			$sales_store_code		= create_sales_store_code();
		...
		case "update_banner_info" :
			...
			$banner_query = "UPDATE ".$_gl['banner_info_table']." SET banner_name='".$banner_name."', device_type='".$device_type."', banner_type='".$banner_type."', banner_showYN='".$banner_showYN."', banner_show_order='".$banner_show_order."', banner_img_link='".$banner_value."', banner_link_target='".$banner_link_target."', banner_regdate='".date("Y-m-d H:i:s")."' WHERE idx='".$banner_idx."'";
		...
		case "update_option_info" :
			...
			$option7_query		= "UPDATE ".$_gl['site_option_table']." SET option_value='".$default_free_delivery_price."', option_load='".$default_free_delivery_priceYN."' WHERE option_name='default_free_delivery_price'"; 
			$option7_result		= mysqli_query($my_db, $option7_query);
		...
		case "change_banner_list" :
			...
			$list_query		= "SELECT * FROM ".$_gl['banner_info_table']." WHERE 1 AND admin_showYN = 'Y' AND banner_type='".$banner_type."' ORDER BY idx DESC";
			$list_result		= mysqli_query($my_db, $list_query);
